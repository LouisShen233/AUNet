...
class LaSOTDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        # Testing sequences are already defined officially.

        self.videos = {}
        for video in pbar:
            # temp = ''.join(re.split(r'[^A-Za-z]', video))
            temp = video.split('-')[0]
            for i in range(len(meta_data[video]['img_names'])):
                meta_data[video]['img_names'][i] = os.path.join(temp, meta_data[video]['img_names'][i])
